...
一台机器上跑 agent 时，技能放在本地目录里就够了。多台一起跑，问题会依次冒出来。
技能改了一端，另一端要等下一次手工复制才更新；谁在什么时候改了哪句话、为什么这么改，
散在各自的终端历史里，三个月后只剩下一句"这段提示词一直是这样"；项目日志记在本地
文件里，换台机器就要重新翻一遍。

nest-sync 把这几件事都交给 Git。Git 本来就是为"多份副本、一条历史线"设计的，它的
版本史、差异比较、冲突检测和离线可用，恰好是上面每个问题的答案。仓库是唯一的事实
来源，其余一切都只是它的包装。
...
| 工作目录用门牌对接 | 合同、素材这类底稿住在各自的项目文件夹里，不进仓库；`.nest` 指针文件记录它对应 `logs/<节点>/`。指针可以嵌套，目录嵌套即节点嵌套。因为那些文件夹根本不是 Git 仓库，底稿物理上推不上云端 |
